...
{
    public class RegistryOfficeSetupMgt
    {
        ...
        public async Task<object> GetByPage(object[] data) //vmCmnParameters cmnParam
        {
            ...
            string listRegistryOffice = string.Empty;
            try
            {
                ht = new Hashtable
                {
                ...
                listRegistryOffice = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_RegistryOfficeByPage, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                Logs.Bug(ex);
        ...
        {
            vmCmnParameter param = JsonConvert.DeserializeObject<vmCmnParameter>(data[0].ToString());
            string objRegistryOffice = string.Empty, objDoc = string.Empty;
            try
            {
                Ora_Generic_vmCmnParameter = new GenericFactoryOracle<vmCmnParameter>();
                ht = new Hashtable
                ...
                objRegistryOffice = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_RegistryOfficeById, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                //Logs.WriteBug(ex);
        ...
        public async Task<object> SaveUpdate(object[] data)
        {
            ...
            try
            {
                ...
                result = await Ora_Generic_vmCmnParameter.ExecuteNonQueryOutString(StoredProcedure.SpSetPut_RegistryOffice, ocmd, "gresult", StaticInfos.conStringOracle.ToString());
            ...
            using (_ctxOra = new ModelContext())
            {
                using (var _ctxOraTran = await _ctxOra.Database.BeginTransactionAsync())
                {
                    try
                    {
                        if (param.id > 0)
                        {
                            var delmodel = await _ctxOra.TRegistryOffices.Where(x => x.Oid == param.strId).FirstOrDefaultAsync();
