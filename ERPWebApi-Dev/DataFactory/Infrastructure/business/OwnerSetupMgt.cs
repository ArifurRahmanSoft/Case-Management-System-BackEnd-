    ...
    public class OwnerSetupMgt
    {
        ...
        public async Task<object> GetByPage(object[] data) //vmCmnParameters cmnParam
        {
            ...
            try
            {
                ...
                listOwner = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_OwnerByPage, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                Logs.Bug(ex);
        ...
        {
            vmCmnParameter param = JsonConvert.DeserializeObject<vmCmnParameter>(data[0].ToString());
            string objOwner = string.Empty, objDoc = string.Empty;
            try
            {
                Ora_Generic_vmCmnParameter = new GenericFactoryOracle<vmCmnParameter>();
                ht = new Hashtable
                ...
                objOwner = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_OwnerById, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                //Logs.WriteBug(ex);
        ...
        public async Task<object> SaveUpdate(object[] data)
        {
            ...
            try
            {
                ...
                result = await Ora_Generic_vmCmnParameter.ExecuteNonQueryOutString(StoredProcedure.SpSetPut_Owner, ocmd, "gresult", StaticInfos.conStringOracle.ToString());
                if (!string.IsNullOrEmpty(result) && result != "0")
                {
                    message = result == "-1" ? "Owner already exists!!!" : MessageConstants.Saved;
            ...
            using (_ctxOra = new ModelContext())
            {
                using (var _ctxOraTran = await _ctxOra.Database.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        {
                            var delmodel = await _ctxOra.TPurchasers.Where(x => x.Oid == param.strId).FirstOrDefaultAsync();
