...
namespace DataFactory.Infrastructure.business.operation
{
    public class ThanaSetupMgt
    {
        ...
        public async Task<object> GetByPage(object[] data) //vmCmnParameters cmnParam
        {
            ...
            try
            {
                ht = new Hashtable
                {
                    ...
                    { "DistrictId", (3, param.id) },
                    { "SearchValue", (4, param.SearchVal) }
                };

                listThana = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_ThanaByPage, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                Logs.Bug(ex);
        ...
        {
            vmCmnParameter param = JsonConvert.DeserializeObject<vmCmnParameter>(data[0].ToString());
            string objThana = string.Empty, objDoc = string.Empty;
            try
            {
                Ora_Generic_vmCmnParameter = new GenericFactoryOracle<vmCmnParameter>();
                ht = new Hashtable
                ...
                objThana = await Ora_Generic_vmCmnParameter.ExecuteCommandString(StoredProcedure.SpGet_ThanaById, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                //Logs.WriteBug(ex);
        ...
        public async Task<object> SaveUpdate(object[] data)
        {
            ...
            try
            {
                ...
                result = await Ora_Generic_vmCmnParameter.ExecuteNonQueryOutString(StoredProcedure.SpSetPut_Thana, ocmd, "gresult", StaticInfos.conStringOracle.ToString());
                if (!string.IsNullOrEmpty(result) && result != "0")
                {
                    message = result == "-1" ? "Thana already exists!!!" : MessageConstants.Saved;
            ...
            using (_ctxOra = new ModelContext())
            {
                using (var _ctxOraTran = await _ctxOra.Database.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        {
                            var delmodel = await _ctxOra.TThanas.Where(x => x.Oid == param.strId).FirstOrDefaultAsync();
