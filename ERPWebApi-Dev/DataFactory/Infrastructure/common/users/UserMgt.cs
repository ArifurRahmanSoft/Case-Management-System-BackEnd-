...
namespace DataFactory.Infrastructure.common.users
{
    public class UserMgt
    {
        ...
        IGenericFactoryOracle<vmCmnParameter> OraGeneric_vmCmnParameter = null;
        ...
        public async Task<object> GetWithPagination(vmCmnParameter param) //vmCmnParameters cmnParam
        {
            ...
            try
            {
                ...
                listUser = await OraGeneric_vmCmnParameter.ExecuteCommandString(StoredProcedure.Ora_SpGet_CmnUser, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                Logs.Bug(ex);
        ...
        //public async Task<object> GetWithPageByUserType(vmCmnParameters cmnParam)
        //{
        //    List<vmUser> listUser = null;
        //    object result = null; int recordsTotal = 0;
        //    try
        //    {
        //        using (_ctx = new dbRGLERPContext())
        //        {
        //            listUser = await (from u in _ctx.CmnUser
        //                              join uwt in _ctx.CmnUserWiseType on u.UserId equals uwt.UserId
        //                              join ut in _ctx.CmnUserType on uwt.UserTypeId equals ut.TypeId
        //                              join r in _ctx.CmnUserRole on u.UserRoleId equals r.RoleId
        //                              join ul in _ctx.CmnUserLogin on u.UserId equals ul.UserId
        //                              where (cmnParam.userType == null || cmnParam.userType == 0 ? 0 : u.UserTypeId) == (cmnParam.userType == null || cmnParam.userType == 0 ? 0 : cmnParam.userType)
        //                              select new vmUser
        //                              {
        //                                  UserId = u.UserId,
        //                                  //FullName=(u.FrstName.Trim()+" "+u.MiddleName.Trim()+" ").Trim()+u.LastName.Trim(),
        ...
        //                                  Password = ul.UserPass
        //                              })
        //                              .OrderBy(a => a.FrstName)
        //                              .Skip(Conversions.Skip(cmnParam))
        //                              .Take((int)cmnParam.pageSize)
        //                              .ToListAsync();

        //            recordsTotal = await _ctx.CmnUser.Where(w => w.UserTypeId == cmnParam.userType).CountAsync();
        ...
        //{
        //    object result = null; CmnUser objUser = null; CmnUserLogin objUserLogin = null; vmCmnUser oVmCmnUser = null;
        //    try
        //    {
        //        using (_ctx = new dbRGLERPContext())
        //        {
        //            var typename = "";
        //            var typeid = "";
        //            objUser = await _ctx.CmnUser.Where(x => x.UserId == id).FirstOrDefaultAsync();
        //            if (await _ctx.CmnUserWiseType.AnyAsync(x => x.UserId == id))
        //            {
        //                var tdata = await _ctx.CmnUserWiseType.Where(x => x.UserId == id).ToListAsync();
        //                //TypeName
        //                typename = String.Join(",", tdata.Select(x => _ctx.CmnUserType.Find((int)x.UserTypeId).TypeName.Trim()));
        //                typeid = String.Join(",", tdata.Select(x => x.UserTypeId.ToString()));


        //            }
        //            objUserLogin = await _ctx.CmnUserLogin.Where(x => x.UserId == id).FirstOrDefaultAsync();
        //            if (objUser != null && objUserLogin != null)
        //            {
        //                oVmCmnUser = new vmCmnUser()
        //                {
        //                    UserId = objUser.UserId,
        //                    FrstName = objUser.FrstName,
        ...
        //                    UserPass = objUserLogin.UserPass,
        //                    IsActive = objUserLogin.IsActive,
        //                    IsLocked = objUserLogin.IsLocked,
        //                    TypeName = typename,
        //                    typeList = typeid.Split(',')
        //                };
        //            }
        ...
        public async Task<object> GetByID(int id)
        {
            ...
            try
            {
                ...
                oVmCmnUser = await Generic_vmCmnParameters.ExecuteCommandString(StoredProcedure.SpGet_cmnUserById, ht, StaticInfos.conString.ToString());
            }
            catch (Exception ex)
            {
                //Logs.WriteBug(ex);
        ...
        public async Task<object> GetEmployeeDetailByID(string EmployeeId)
        {
            ...
            try
            {
                ...
                empDetail = await OraGeneric_vmCmnParameter.ExecuteCommandString(StoredProcedure.Ora_SpGet_EmployeeById, ht, StaticInfos.conStringOracle.ToString());
            }
            catch (Exception ex)
            {
                ex.ToString();
        ...
        public async Task<object> SaveUpdate(string _JsonData, vmCmnParameter param)
        {
            ...
            try
            {
                ...
                result = await OraGeneric_vmCmnParameter.ExecuteNonQueryOutString(StoredProcedure.Ora_SpSet_CmnUser, ocmd, "mresult", StaticInfos.conStringOracle.ToString());
        ...
        //    public DataRowVersion? srcVersion { get; set; }
        ...
        {
            object result = null; string data = string.Empty; string Message = string.Empty; bool resstate = false, isByte = true;

            string getUrl = StaticInfos.loggedInfoUrl + param.UserID;

            if (param.IsTrue)
            {
                data = getLoggedUserInternalInfo(param.UserID);
                resstate = MessageConstants.SuccessState;
            }
            else
            {
        ...
        private string getLoggedUserInternalInfo(string userId)
        {
            string data = string.Empty;
            using (_ctxOra = new ModelContext())
            {
                var dd = (from su in _ctxOra.TSysUsers
                          where su.Userid.ToLower() == userId.Trim().ToLower()
                          select new
                          {
                ...
                data = JsonConvert.SerializeObject(dd.Result);
            }
