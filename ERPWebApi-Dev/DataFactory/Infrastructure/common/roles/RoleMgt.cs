...
namespace DataFactory.Infrastructure.common.roles
{
    public class RoleMgt
    {
        ...
        {
            object result = null, objRole = null;
            try
            {
                using (var _ctxOra = new ModelContext())
                {
                    ...
                    //objRole = await _ctx.CmnUserRole.Where(x => x.RoleId == id).FirstOrDefaultAsync();
                }
            }
            catch (Exception ex)
            {
